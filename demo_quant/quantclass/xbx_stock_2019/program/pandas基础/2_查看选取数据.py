...
df = pd.read_csv(
    ...
    filepath_or_buffer=r'/demo_quant/quantclass/xbx_stock_2019/data/sh600000.csv',
...
# print(df.head(3000))  # 看前3行的数据，默认是5。与自然语言很接近
...
# 对print出的数据格式进行修正
# pd.set_option('expand_frame_repr', False)  # 当列太多时显示不清楚
# pd.set_option("display.max_rows", 5000)  # 设定显示最大的行数
...
# print(df.loc['1999-11-10':'1999-11-19'])  # 选取在此范围内的多行，和在list中slice操作类似，读取的数据是DataFrame类型
# print(df.loc[:, '开盘价':'最低价'])  # 选取在此范围内的多列，读取的数据是DataFrame类型
# print(df.loc['1999-11-10': '1999-11-19', '开盘价':'收盘价'])  # 读取指定的多行、多列。逗号之前是行的范围，逗号之后是列的范围。读取的数据是DataFrame类型
# print(df.loc[:, :])  # 读取所有行、所有列，读取的数据是DataFrame类型
# print(df.at['1999-11-10', '开盘价'])  # 使用at读取指定的某个元素。loc也行，但是at更高效。
# print(df.loc['1999-11-10', '开盘价'])
